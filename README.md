...
The `--name` value becomes the dep's name and install path: `--name tools` installs at `<vendor>/tools`, `--name tool-a/util` installs at `<vendor>/tool-a/util`. To rename a dep, `graft remove` it and `graft add` with the new `--name`.
