...
| 欄位 | 必需 | 描述 |
|-------|----------|-------------|
| `dir` | 是 | 已安裝依賴的根目錄。預設值為 `"deps"`（由 `graft init` 設定，§4.1）；清單缺少此欄位是驗證錯誤（結束碼 2）。必須是儲存庫內的相對路徑；`"."`（儲存庫根目錄）、含 `..` 的路徑、以及含 `.git` 區段的路徑（如 `.git`、`vendor/.git`）會被拒絕（結束碼 2），因為同步流程會刪除 `<dir>` 下不屬於任何依賴的路徑——若 `<dir>` 與 `.git` 重疊將摧毀儲存庫。注意：`vendor/` 在某些生態系中有特殊意義（Go 的 `go mod vendor`、PHP Composer）——這些生態系的專案應改用其他名稱。 |

**`[[deps]]` 欄位參考**

| 欄位 | 必需 | 描述 |
|-------|----------|-------------|
| `name` | 是 | 本地識別符與安裝路徑。必須唯一。每個斜線分隔的段必須符合 `[A-Za-z0-9._-]+`。簡單名稱（`tools`）安裝至 `<dir>/tools`；路徑型名稱（`tool-a/util`）安裝至 `<dir>/tool-a/util`。當 `graft add` 未帶 `--name` 時，預設為 `repo` 的最後一段路徑（去除 `.git` 後綴）。`--name` 旗標直接設定名稱。名稱是條目的主鍵；同一個 `repo` 可以出現在多個條目（條目指認與撞名規則見 §4.2）。名稱形成祖先/後代路徑的兩個條目（如 `foo` 與 `foo/bar`）會因安裝路徑重疊而衝突——這是驗證錯誤（結束碼 2）。名稱的第一段也不得以 `.graft-` 前綴開頭（以不分大小寫比對，因此像 `.GRAFT-TMP` 這種在大小寫不敏感檔案系統上會撞名的變體在所有平台都會被拒絕）：該前綴保留給 graft 在 vendor 根目錄下的內部目錄使用，這些目錄一律命名為 `.graft-<something>`（例如同步暫存目錄 `.graft-tmp`），這樣的名稱會與其中之一衝突而永遠無法套用——這同樣是驗證錯誤（結束碼 2）。含 `.` 的名稱（如 `github.com/org/repo`，第一段是 `github.com`）不受影響，沒有連字號的 `.graft` 本身也不受影響。 |
...
具體來說：對已安裝樹中的每個檔案，以 UTF-8 檔案路徑加換行符、一個 exec 位元組（`\x00` 代表不可執行，`\x01` 代表可執行），再加原始檔案位元組計算 `sha256`，以 64 個小寫十六進位字元編碼結果；將所有每檔的十六進位字串以 ASCII byte 順序（即對 64 字元小寫 hex 字串的字典序）排序後，再對其拼接結果計算最終的 `sha256`，同樣以 64 個小寫十六進位字元編碼。`hash` 欄位的儲存格式為 `sha256:` 前綴加 64 個小寫 hex 字元（例如 `sha256:e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855`）；`sha256:` 前綴是儲存值的一部分。這讓 `graft apply` 和 `graft status` 能偵測到鎖定內容與 vendor 檔案內容之間的任何差異——例如被手動修改的 vendor 檔案——並明確報錯。（exec 位元組取自記錄的 git 中繼資料，而非檔案系統當下的模式，詳見下方。）
...
**同一 repo 宣告多次。** 清單允許同一個 `repo` 出現在多個條目——典型情境是從 monorepo 取多個子目錄，各帶不同的 `subdir`。每個條目以 `name` 為主鍵獨立解析與鎖定（可以鎖在不同版本），並共用同一個快取裸儲存庫（§5.4）。新增第二個條目時必須以 `--name` 指定未被占用的名稱——否則依上述規則，repo 比對會落在第一個條目而變成更新它。之後的版本更新透過再次傳入 repo URL 進行（同 repo 多條目時搭配 `--name` 指明目標）。
...
改寫 `graft.toml` 中自己的條目後，`add` 會以完整的 `graft lock` 語義收尾——重新同步*每一個*條目，而不只是被改動的那個，因此對其他依賴的手動編輯會在同一次執行中被一併處理——接著執行與 `graft apply` 相同的同步流程：vendor 目錄會被帶到與鎖定檔完全一致的狀態——補上缺少的依賴、移除多餘的、對齊不符的。因此 `add` 永遠不會因既存的 toml ↔ lock 不一致而失敗；它會直接解決這個不一致。當這次重新同步改動到目標以外的任何依賴時，`add` 會印出 `also synced other dependencies:`，接著列出受影響的安裝／移除行，讓連帶變更不會被誤認為是本次請求更新的一部分。
...
- 對每個清單條目比對鎖定檔中對應條目的 `repo`、`version`、`subdir`、`symlinks`；對每個鎖定條目確認仍存在於清單。任一不符（新增、移除或欄位變更）→ 以結束碼 2 退出，並逐條列出待更新的依賴名稱，提示執行 `graft lock` 再提交。
...
全域快取（§5.4）在一般使用中是隱形的；以下子命令用於管理它：
...
| `GRAFT_LINK_MODE` | `copy` | 控制具現化模式（§5.4）：`copy`（預設）或 `symlink`。這是每台機器的選擇；所有具現化命令（`apply`、`add`、`remove`）一視同仁地遵循它，永遠不會記錄在 `graft.toml` 或 `graft.lock` 中。 |
...
2. **安裝階段**（CPU / IO 綁定）：worker 數量為 `min(numDeps, CPU 核心數)`。每個 worker 將 store 中已備妥的樹以原子 rename（或 copy-on-write reflink）移入 `<dest>`。

兩階段均在完成後收集所有錯誤一起輸出（不採用快速失敗，因此可以一次看到所有錯誤）。同一個裸儲存庫的多個依賴仍由每 repo 的 advisory lock（§5.4）序列化擷取。

### 5.3 擷取策略

所有擷取都以該依賴的裸快取儲存庫（§5.4）為目標，且為增量擷取——快取中已存在的 commit 永遠不會被重新擷取。直接以 commit SHA 擷取（`git fetch <repo> <commit>`）只有在伺服器允許時才可行（`uploadpack.allowReachableSHA1InWant` 或 `allowAnySHA1InWant`）。GitHub、GitLab 與 Gitea 都有開啟；單純的 `git daemon` 或較舊的伺服器可能沒有。因此 graft 依序嘗試：
...
└── locks/                          # advisory 鎖：每儲存庫擷取鎖 + 每專案修改鎖（§5.5）
```
...
- **快取側**（§5.4）：每儲存庫擷取鎖；store 條目不可變且以原子 rename 建立——不需要整個快取範圍的全域鎖。
...
**可寫回性（TOML 安全）。** `graft add` 與 `graft remove` 以就地方式重寫 `graft.toml`（§3.1），欄位值以 TOML basic string 原樣輸出、不做跳脫；因此 `repo`、`version`、`subdir` 與頂層 `dir` 中的雙引號（`"`）、反斜線（`\`）與控制字元在 `graft add` 與載入時即以結束碼 2 拒絕——git 本身允許 tag 名稱含其中部分字元，所以 ref 並非天生安全——確保 graft 寫出的清單永遠可以再被解析（`name` 已由 §3.1 的字元白名單涵蓋）。
...
**整合測試**使用本地裸 git 儲存庫作為 fixture 遠端，在暫存目錄下即時建立。無需網路存取。每個測試建立一個暫時工作目錄，以子行程執行 graft 指令，並驗證檔案內容與結束碼。
...
### 里程碑 3 — 打磨
- 平行安裝（worker 池）
- 會修改狀態的命令取得每專案 advisory lock（§5.5）
- 所有錯誤路徑的清晰錯誤訊息
- 安裝腳本（`curl | sh`）
