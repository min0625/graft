...
**路徑安全。** `dir` 必須是儲存庫內的相對路徑；`name` 指定 `<dir>` 之下的路徑（`subdir` 則選遠端 repo 的子目錄）——絕對路徑、含 `..` 的路徑、以及含 `.git` 區段的路徑（否則具破壞性的 vendor 同步會與 git 儲存庫重疊）在驗證階段即被拒絕（結束碼 2）；`name` 的第一段也不得為同步暫存目錄 `.graft-tmp`（否則安裝路徑會與暫存目錄重疊而永遠無法套用），同樣在驗證階段被拒絕（結束碼 2）；解析後的完整安裝路徑（`<dir>/<name>`）永遠在安裝目錄之下，因此惡意或損壞的清單／鎖定檔永遠無法把安裝或同步刪除導向安裝目錄之外。在擷取的檔案樹內，git 本身就拒絕追蹤包含 `..` 或 `.git` 的路徑，所以惡意依賴也無法逃出自己的安裝根目錄。
