...
lock_version = 1
vendor = "vendor"                                          # install root, copied from graft.toml
...
| `vendor` | The install root, copied from `graft.toml`. Recorded once at the top level so that `graft apply` can rely on the lockfile alone: each dep's install path is `<vendor>/<name>`, which is also how `apply` knows which paths it owns when removing surplus dependencies. |
| `name` | Corresponds to `name` in `graft.toml`. Together with the top-level `vendor` it fully determines the install path (`<vendor>/<name>`). |
...
**Project root discovery.** Except for `init` and `cache`, every command walks upward from the current working directory to the nearest directory containing a `graft.toml` — the project root — and runs as if invoked there: relative paths resolve against the project root (`vendor` is project-root relative; each dependency installs at `<vendor>/<name>`). The upward walk never crosses a git repository boundary (the directory containing `.git` is the last one tried). When no `graft.toml` is found, the command fails with exit code 2 and the hint: `graft.toml not found. Run 'graft init <vendor>' first.`
...
**`--name` flag.** Sets the dep's `name` (and therefore its install path under `<vendor>`). The full value — including any `/` — becomes the entry name. A simple name (`--name tools`) installs at `<vendor>/tools`; a path-like name (`--name tool-a/util`) installs at `<vendor>/tool-a/util`.
...
type Lockfile struct {
    ...
    Vendor      string       `toml:"vendor"` // install root, copied from graft.toml
...
**Path safety.** `vendor` must be a relative path inside the repository; `name` names a path under `<vendor>` (and `path` selects a subdirectory of the remote repo) — absolute paths and `..` segments are rejected at the validation stage (exit code 2); the fully-resolved install path (`<vendor>/<name>`) always lands inside the vendor tree, so a malicious or corrupt manifest/lockfile can never direct an install, or a reconcile delete, outside it. Within the fetched file tree, git itself refuses to track paths containing `..` or `.git`, so a malicious dependency cannot escape its own install root either.
