...
- `graft cache clean` — remove store entries with no registered link-mode dest referencing them, plus bare repositories not fetched recently. The cache is always safe to delete, so to wipe it entirely just remove the directory printed by `graft cache dir`. Cleaning may break other projects' link-mode vendors: affected dependencies show as `missing` in `graft status`, and `graft apply` will re-materialize them (re-fetching if necessary).
