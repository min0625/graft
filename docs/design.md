...
```toml
vendor = "vendor"           # required — the root directory deps install into, set at `graft init`
...
|-------|----------|-------------|
| `vendor` | yes | Root directory for installed dependencies. No default — this value is set explicitly via `graft init <vendor>` (§4.1); a manifest missing this field is a validation error (exit code 2). It must be a relative path inside the repository; `"."` (the repo root) and paths containing `..` are rejected (exit code 2), because the reconcile step deletes any path under `<vendor>` that belongs to no dependency. Note: `vendor/` has special meaning in some ecosystems (Go's `go mod vendor`, PHP Composer) — projects in those ecosystems should use a different name such as `deps`. |

**`[[deps]]` field reference**

| Field | Required | Description |
|-------|----------|-------------|
| `name` | yes | Local identifier. Must be unique and must match `[A-Za-z0-9._-]+` — in particular it never contains `/`, so a name can always be told apart from a repo path in `graft add`. When `graft add` is run without `--name`, it defaults to the last path segment of `repo` (with any `.git` suffix stripped). The name is the entry's primary key; the same `repo` may appear in multiple entries (entry resolution and name-collision rules are in §4.2). |
| `repo` | yes | A scheme-less repository path (`github.com/org/repo`, fetched over HTTPS), or an explicit `https://` / SSH URL. |
| `version` | yes | The locked version, go.mod-style: a git tag when one exists (`"v1.2.0"`), otherwise a pseudo-version (see below). Written by `graft add`; safe to hand-edit — change it to a new tag and run `graft lock`. The resolved commit SHA lives only in `graft.lock`. A remote re-resolution happens only when a dependency's `repo` or `version` changes — changing only `path` or `dest` never triggers a ref lookup — so a re-pointed tag can never silently change what gets installed (see §7). |
...
| `dest` | no | Local destination path. Defaults to `<vendor>/<name>`. Must be a relative path inside the repository (absolute paths and `..` are not accepted). Two dependencies must not resolve to the same or nested `dest` paths — that is a validation error (exit code 2). |
...
dest    = "vendor/shared-scripts"                         # resolved install path
...
| Field | Description |
|-------|-------------|
| `lock_version` | Format version, currently fixed at `1`. Used to detect future breaking changes. |
| `name` | Corresponds to `name` in `graft.toml`. |
...
| `dest` | The fully resolved local install path. Recorded here so that `graft apply` can rely on the lockfile alone — including knowing which paths it owns when removing surplus dependencies or after a moved/renamed `dest`. |
...
`sha256(sort(sha256(filepath + "\n" + content) for each file in the tree))`

Concretely: for each file in the installed tree, compute `sha256` over the UTF-8 file path plus a newline plus the raw file bytes; sort all the resulting hex strings alphabetically; then compute the final `sha256` over their concatenation. This lets `graft apply` and `graft status` detect any difference between the locked content and what is actually on disk — including a hand-edited vendor file — and report it explicitly.
...
- File modes — including the executable bit — are not part of the hash. Windows filesystems cannot represent the executable bit, and including it would make on-disk verification impossible on Windows. On POSIX systems graft preserves the mode git records at checkout, but a mode-only change is not detected as drift. This is a documented limitation.
...
|--------|--------------|---------------|-----------------|-------------|
| `init` | creates | — | — | Create `graft.toml` (`graft init <vendor>`) |
...
`graft init <vendor>` takes the vendor root as a required argument (e.g. `graft init deps`) — there is no default, so the choice is always deliberate. It creates `graft.toml` in the current directory; it fails with exit code 2 when the argument is missing or `graft.toml` already exists — it never silently overwrites.

**Project root discovery.** Except for `init` and `cache`, every command walks upward from the current working directory to the nearest directory containing a `graft.toml` — the project root — and runs as if invoked there: all relative paths (`vendor`, `dest`) resolve against the project root. The upward walk never crosses a git repository boundary (the directory containing `.git` is the last one tried). When no `graft.toml` is found, the command fails with exit code 2 and the hint: `graft.toml not found. Run 'graft init <vendor>' first.`
...
For a dependency that already exists in `graft.toml`, the first argument may also be its `name` instead of a repo URL — for example `graft add shared-scripts@v1.3.0`. Using an unknown name (neither an existing dependency name nor a valid repo URL) reports an error. The two forms are never confused: a name must not contain `/`, while a repo path always does.

When updating an existing dependency, any `--name`, `--dest`, or `--path` flag that is passed replaces the existing value; a flag that is not passed keeps the original.
...
- First argument is the **name form** → target the entry with that name (does not exist → error, as above).
- First argument is the **repo form** with `--name` → target the entry with that name: update it if it exists (the repo argument becomes the new `repo` — giving both a name and a repo explicitly is treated as a deliberate re-point); add it if it does not.
- First argument is the **repo form** without `--name` → match existing entries by normalized repo (canonical form, see §5.6):
  - exactly one entry → update that entry, keeping its name (even a custom one).
  - more than one entry (the same repo declared multiple times) → error: list the matching names, suggesting `graft add <name>@<ref>` or `--name` to disambiguate.
...
- For each dependency it reports one of the following states:
  ...
  - `extra` — a path under `<vendor>` that belongs to no locked dependency (a leftover from a removed dependency, or created by hand). `graft apply` will delete it. A toml ↔ lock disagreement is always reported as `out of sync`, never `extra`. When the lockfile does not exist, `extra` is not reported — everything is already `out of sync`, and an extra report would just be noise.
...
type Manifest struct {
    Vendor string `toml:"vendor"` // required; set at `graft init <vendor>`
    Deps   []Dep  `toml:"deps"`
}
...
type LockedDep struct {
    ...
    Dest    string    `toml:"dest"`           // resolved install path
...
for each dependency (parallel, N workers):
  ...
  (copy mode: reflink/copy staged under <vendor>/.graft-tmp then rename;
   link mode: create a symlink / junction)
     │
     ▼
remove items under <vendor> matching no locked dest
(a custom dest outside <vendor> is never auto-cleaned)
     │
     ▼
print summary
```

The checkout staging area lives in `<cache>/tmp/`, on the same filesystem as the store, so the rename into `store/` is atomic; if another process is building the same entry concurrently, the loser of the rename race simply uses the existing entry. Copy-mode materialization is staged under `<vendor>/.graft-tmp/` rather than the system temp directory — so that, in the common case, the final move into `<dest>` is an atomic same-filesystem rename. If `dest` is on a different filesystem (possible with a custom `dest`), graft falls back to copy + delete. Leftover items from an interrupted run in either staging area are cleaned up on the next state-modifying command, and `.graft-tmp` is never treated as a surplus dependency during reconcile.

### 5.4 Parallelism

Installs use a worker pool bounded by `min(numDeps, runtime.NumCPU())`. Each worker handles one dependency: clone → hash → move. Errors are collected and reported together after all workers finish (no fail-fast, so you see every error at once).
...
**Path safety.** `vendor` and `dest` must be relative paths inside the repository — absolute paths and `..` segments are rejected at the validation stage (exit code 2), so a malicious or corrupt manifest/lockfile can never direct an install, or a reconcile delete, outside the repository. Within the fetched file tree, git itself refuses to track paths containing `..` or `.git`, so a malicious dependency cannot escape its own install root either.
