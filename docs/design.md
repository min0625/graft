...
Checkouts always force `-c core.autocrlf=false -c core.eol=lf` (byte normalization, §3.2), `-c core.symlinks=false` (symlink policy, §3.2), and `-c core.longpaths=true` — the last so that deep paths exceeding Windows' traditional `MAX_PATH` (260 characters) limit still check out.
