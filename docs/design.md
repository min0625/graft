...
**Materialization.** How a store entry becomes `<dest>` is selected by `graft apply --link-mode=<mode>` (or `GRAFT_LINK_MODE=<mode>`); the flag overrides the environment variable. It is a machine-local choice, never recorded in `graft.toml` or `graft.lock`. The vocabulary (`copy`, `symlink`) is a subset of uv's link modes, leaving room to add `hardlink`/`clone` later; two are implemented today:
