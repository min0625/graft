...
| REQ-EXIT-NET | A remote that cannot be reached fails with exit code 3 (network error). | §4.6, §5.3 |
...
| REQ-APPLY-STAGING-CLEANUP | Every reconcile removes the staging directory `<vendor>/.graft-tmp` before returning — on failure as well as on success. | §5.1 |
...
| REQ-REPO-WHITESPACE | A dependency `repo` containing any whitespace (space, tab, newline, or other Unicode whitespace) is rejected by `graft add` and at load with exit code 2, before any network access — so an obviously-malformed value is not misreported as an exit-3 network error when git rejects it as "malformed input". | §7 |
| REQ-PARALLEL-COLLECT | The parallel reconcile collects and reports every dependency's error, rather than failing fast on the first. | §5.2 |
| REQ-JOBS-FETCHDEFAULT | The fetch phase runs up to 16 concurrent workers, which can exceed `runtime.NumCPU()`; the install phase runs up to `runtime.NumCPU()` workers. | §5.2 |
...
| REQ-APPLY-CWD-GUARD | Before renaming or removing a dest (reinstalling a dep, clearing stale staging, deleting an extra), `graft apply` fails with exit code 1 and a message naming the attempted action ("replace"/"remove") when the process's current working directory is at or inside that path, instead of surfacing the underlying OS error — on every OS, not only Windows. A symlink/junction dest is exempt: replacing or removing the link node never disturbs a cwd that resolves through its target. | §5.1 |
| REQ-FETCH-NOPROMPT | Every `git` invocation sets `GIT_TERMINAL_PROMPT=0` and the command-line override `-c credential.interactive=false`, which cannot collide with or shadow the caller's own `GIT_CONFIG_*` credential-rewrite entries (e.g. `url.<base>.insteadOf`), so a private HTTPS repo with no cached credentials fails fast instead of hanging on an interactive credential-helper prompt. | §7 |
| REQ-STATUS-SYMLINKS-SYNC | `graft status` reports `out of sync` (exit 2) when a dependency's `symlinks` policy differs between manifest and lockfile — status judges sync with the same key set (`version`, `repo`, `subdir`, `symlinks`, `dest`) as `graft apply` and `graft lock --check`, so an all-`ok` status implies `apply` will not refuse to run. | §4.5 |
| REQ-TOML-SAFE | A dep `repo`, `version`, or `subdir`, or the top-level `dir`, containing a double quote, backslash, or control character is rejected by `graft add` and at load with exit code 2, so the comment-preserving surgical `graft.toml` writer — which emits values verbatim inside a TOML basic string — can never produce a manifest that no longer parses. | §3.1, §7 |
| REQ-EXECBIT-QUOTEPATH | Exec-bit detection reads paths from git verbatim (`core.quotePath=off`), so an executable file with a non-ASCII path is recorded under its real name in `.graft-execbits` and its exec bit participates in the content hash, instead of being silently dropped via git's octal-escaped quoting. | §3.2 |
| REQ-FETCH-SYMLINK-MODE | The symlink policy (`reject` and `skip`) is decided from git tree-object modes (`120000`) before checkout, never from the checked-out filesystem, and graft's checkouts force `core.symlinks=false` — so rejection, skipping, and the content hash are identical on every platform regardless of git's `core.symlinks` setting, and a skipped symlink's placeholder file never reaches the hash, the store, or the vendor directory. | §3.2 |
| REQ-FETCH-GITLINK | A dependency tree containing a git-submodule entry (mode `160000`) is rejected with exit code 2 naming the entry's path, because a plain checkout would silently omit the submodule's content. | §3.2 |
| REQ-FETCH-PATHCHECK | Path portability validation and cross-platform collision detection run on the git tree-object paths before checkout, so they fire identically on case-insensitive filesystems, where colliding paths have already overwritten each other by the time the checked-out tree could be inspected. | §3.2 |
| REQ-FETCH-LONGPATHS | graft's checkouts force `core.longpaths=true`, so a dependency tree containing paths longer than the traditional Windows `MAX_PATH` (260 characters) still locks and installs on Windows. | §5.3 |
| REQ-EXIT-MAXCODE | When a single run collects several errors (e.g. a parallel reconcile gathering one failure per dependency), the process exits with the highest exit code among them, independent of dependency processing order — an integrity failure (4) is never masked by a network error (3). | §4.6 |
